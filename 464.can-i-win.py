...
#
class Solution:
    def A(self, nums, goal):
        for each in nums:
            if each > goal:
                return True
        for i in range(len(nums)):
            if self.B(nums[:i]+nums[i+1:], goal-nums[i]):
                return False
        return True
    def B(self, nums, goal):
        for each in nums:
            if each > goal:
                return True
        for i in range(len(nums)):
            if self.A(nums[:i]+nums[i+1:], goal-nums[i]):
                return False
        return True
    def canIWin(self, maxChoosableInteger: int, desiredTotal: int) -> bool:
        nums = [each for each in range(1,maxChoosableInteger+1)]
        return self.A(nums,desiredTotal)
